...
class GameState:
    ...
    def getWinner(self) -> 'GameState.GamePlayer':
        ...
        elif self.p2points >= 25:
            return self.GamePlayer.p2
        elif sum(self.p1pad) == 0 or sum(self.p2pad) == 0:
            return self.GamePlayer.p1 if self.p1points > self.p2points else self.GamePlayer.p2
        else:
            return None
    ...
    def simulate(self, player: 'GameState.GamePlayer', cavityIndex: int) -> 'GameState':
        ...
        # Vérifier si le joueur actuel a affamé son adversaire
        opponent = self.GamePlayer.p1 if player == self.GamePlayer.p2 else self.GamePlayer.p2
        if (opponent == self.GamePlayer.p1 and sum(new_state.p1pad) == 0) or (opponent == self.GamePlayer.p2 and sum(new_state.p2pad) == 0):
            # Vérifier si le joueur actuel peut nourrir son adversaire
            if opponent == self.GamePlayer.p1:
                if any(new_state.simulate(player, move).p1pad for move in range(len(new_state.p2pad)) if new_state.p2pad[move] > 0):
                    return new_state  # Le joueur peut nourrir son adversaire, continuer le jeu
                else:
                    new_state.p2points += sum(new_state.p2pad) + sum(new_state.p1pad)
                    new_state.p1pad = [0] * 6
                    new_state.p2pad = [0] * 6
                    return new_state
            else:
                if any(new_state.simulate(player, move).p2pad for move in range(len(new_state.p1pad)) if new_state.p1pad[move] > 0):
                    return new_state  # Le joueur peut nourrir son adversaire, continuer le jeu
                else:
                    new_state.p1points += sum(new_state.p1pad) + sum(new_state.p2pad)
                    new_state.p1pad = [0] * 6
                    new_state.p2pad = [0] * 6
                    return new_state

        # Si le joueur actuel n'a pas affamé son adversaire, retourner le nouvel état
        return new_state
    _infinty = 0xFFFFFFFF
    _invalidMove = -100
...
def displayGameState(state: GameState):
    ...
    print("│  (0)    │  (1)   │  (2)   │  (3)   │  (4)   │  (5)   │  P   │")
    print("├─────────┼────────┼────────┼────────┼────────┼────────┼──────┤")
    print(f"│  {state.p1pad[0]:^7}│{state.p1pad[1]:^7} │{state.p1pad[2]:^7} │{state.p1pad[3]:^7} │{state.p1pad[4]:^7} │{state.p1pad[5]:^7} │ p1   │")
    print(f"│  {state.p2pad[0]:^7}│{state.p2pad[1]:^7} │{state.p2pad[2]:^7} │{state.p2pad[3]:^7} │{state.p2pad[4]:^7} │{state.p2pad[5]:^7} │ p2   │")
    print("╰─────────┴────────┴────────┴────────┴────────┴────────┴──────╯")
    print("╭────────────┬──────────╮")
...
def main():
    ...
    player = GameState.GamePlayer.p2  # Human player
    opposite = GameState.GamePlayer.p1  # AI player
