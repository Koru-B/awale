...
class GameState:
    ...
    def simulate(self, player: 'GameState.GamePlayer', cavityIndex: int) -> 'GameState':
        ...
        if (opponent == self.GamePlayer.p1 and sum(new_state.p1pad) == 0) or (opponent == self.GamePlayer.p2 and sum(new_state.p2pad) == 0):
            # Si le joueur adverse est affamé, il reçoit toutes les graines du joueur actuel
            if opponent == self.GamePlayer.p1:
                new_state.p1points += sum(new_state.p1pad)
                new_state.p1pad = [0] * 6
            else:
                new_state.p2points += sum(new_state.p2pad)
                new_state.p2pad = [0] * 6
            return new_state
